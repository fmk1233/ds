...
(function($, window) {
	...
	var View = $.Class.extend({
			...
			for (; target && target !== document; target = target.parentNode) {
				if (target.tagName === 'A' && target.hash) {
					...
					if (page && page.classList.contains(CLASS_PAGE)) {
                        ...
                        var s={};$.each(target.dataset,function (i,t) {
                            s[i] = t;
                        });
                        $.trigger(page,'viewDidLoad',s);
						this.go(target.hash);
						break;
					}
				}
		...
		fire:function (pageSelector,params) {
            $.trigger(document.querySelector(pageSelector),'viewDidLoad',params||{});
            this.go(pageSelector);
        },
