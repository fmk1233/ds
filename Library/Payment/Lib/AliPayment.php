...
class AliPayment
{
    ...
    public function __construct(array $config)
    {
        ...
        $config['signType'] && $this->aopClient->signType = $config['signType'];
    }
    ...
    function check($arr)
    {
        if ($this->aopClient->signType == 'RSA') {
            return $this->aopClient->rsaCheckV1($arr, $this->aopClient->alipayPublicKey);
        } else {
            return $this->aopClient->rsaCheckV2($arr, $this->aopClient->alipayPublicKey);
        }
     ...
     * @throws Exception
     */
    public function fundTransToaccountTransferRequest()
    {
        $content['out_biz_no'] = '3142321523432';
        $content['payee_type'] = 'ALIPAY_LOGONID';
        $content['payee_account'] = 'bdluyv4362@sandbox.com';
        $content['amount'] = 12.23;
        $content['payer_show_name'] = '上海交通卡退款';
        $content['payee_real_name'] = '沙箱环境';
        $content['remark'] = '转账备注';
