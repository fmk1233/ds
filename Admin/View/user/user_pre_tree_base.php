...
<?php if($member): ?>
    <?php $grade_ary = Common_Function::getRankName();$user_id = Common_Function::user_id(); if($member['rid']&&($user_id==0||$member['id']!=$user_id)){?>
        <div align="center" class="Register_a">
            <a href="javascript:void(-1)" member data-id="<?php echo $member['rid'] ; ?>"><input type="button" name="btnSearch" value="<?php echo T('上一层'); ?>" style="margin-bottom: 20px;" id="btnSearch" ></a>
        </div>
    <?php }?>
    <?php
    switch($member['rank']){
    ...
    <ol class="dd-list">
        <li class="dd-item" data-id="<?php echo $member['id']; ?>">
            ...
            <?php function tuopu_tree($member_id,$dept,$dept1,$type){
                ...
                if ($dept>=$dept1) {$i=1; $c_num = POSNUM;
                    for($m=1;$m<=$c_num;$m++){
                ...
                <?php  }else{?>
                            <li class="dd-item">
                                <div class="dd-nodrag">
                                    <span class="label label-info"><i class="fa fa-user"></i></span> <a href="<?php echo $type=='user'?'?service=User.UserReg':'?service=DUser.UserReg','&pos=',$i,'&user_rid=',$member_id;?>"><?php echo T('注册'); ?></a>
                                </div>
                            </li>
