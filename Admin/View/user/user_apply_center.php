...
<script type="text/javascript">
    ...
    $(function () {
        ...
        var rank_names = JSON.parse('<?php echo json_encode(Common_Function::getBdCenterName()); ?>');
//        var up_names = JSON.parse('<?php //echo json_encode(Common_Function::upgradeName()); ?>//');
        ...
        var $columnsLock = [{
            ...
            formatter:function (value,d) {
                var $html = '';
                if(d.status==0){
                    $html +='<a class="btn btn-danger btn-outline btn-xs" refuse data-service="DUser.DealApplyCenter"  data-action="refuse" data-id="'+d.id+'"><i class="fa fa-ban"></i> 拒绝</a> ';
                    $html +='<a class="btn btn-primary btn-outline btn-xs" pass data-action="pass" data-service="DUser.DealApplyCenter"  data-id="'+d.id+'"><i class="fa fa-check"></i> 通过</a> ';
                }else{
                    $html +='-';
                }
                return $html;
        ...
        var querystrLock = "{service:'DUser.GetApplyCenterList',qtype:$('#qtype').val(),qvalue:$('#qvalue').val(),s_time:$('#s_time').val(),e_time:$('#e_time').val()}";
        var optionsLock = {
            columns: $columnsLock,
            exportOptions: {fileName: '报单中心申请信息'}
        };
        ...
        $('.table').on('click', 'a[refuse]', function () {
            var $this = $(this);
            var data = $this.data();
            confirmMsg('您确认要拒绝该申请订单',function(){
        ...
        $('.table').on('click', 'a[pass]', function () {
            var $this = $(this);
            var data = $this.data();
            confirmMsg('您确认要通过该申请订单',function(){
                sendButtonAjax($this,data,{
                    callback: function (d) {
                        if (d.code == 40000) {
                            var row = $('.table tr[data-uniqueid="' + data.id + '"]');
