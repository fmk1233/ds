        ...
        if (isset($rule['min']) && isset($rule['max']) && $rule['min'] > $rule['max']) {
            throw new PhalApi_Exception_InternalServerError(
                T('min should <= max, but now {name} min = {min} and max = {max}', 
                    array('name' => isset($rule['desc'])?$rule['desc']:$rule['name'], 'min' => $rule['min'], 'max' => $rule['max']))
            );
        }
    }
        ...
        if (isset($rule['min']) && $value < $rule['min']) {
            throw new PhalApi_Exception_BadRequest(
                T('{name} should >= {min}, but now {name} = {value}', 
                    array('name' => isset($rule['desc'])?$rule['desc']:$rule['name'], 'min' => $rule['min'], 'value' => $value))
            );
        }
    }
    ...
    protected function filterRangeCheckMax($value, $rule) {
        if (isset($rule['max']) && $value > $rule['max']) {
            throw new PhalApi_Exception_BadRequest(
                ...
                array('name' => isset($rule['desc'])?$rule['desc']:$rule['name'], 'max' => $rule['max'], 'value' => $value))
            );
        }
    }
        ...
        if (!in_array($value, $rule['range'])) {
            throw new PhalApi_Exception_BadRequest(
                T('{name} should be in {range}, but now {name} = {value}', 
                    array('name' => isset($rule['desc'])?$rule['desc']:$rule['name'], 'range' => implode('/', $rule['range']), 'value' => $value))
            );
        }
    }
}
