...
class PhalApi_Request_Formatter_String extends PhalApi_Request_Formatter_Base implements PhalApi_Request_Formatter {
    ...
    public function parse($value, $rule) {
        ...
        //过滤html, php标签
        $value = $this->filterHtml($value,$rule);
    ...
    /**
     * 根据hmlt规则选择只过滤html标签，还是过滤html标签的php标签
     */
    protected function filterHtml($value, $rule)
    {
        if(isset($rule['html'])){
    ...
    protected function filterByRegex($value, $rule) {
        ...
        if (preg_match($rule['regex'], $value) <= 0) {
            throw new PhalApi_Exception_BadRequest(T('{name} can not match {regex}', array('name'  => isset($rule['desc'])?$rule['desc']:$rule['name'],
                                                                                           'regex' => $rule['regex']
                )));
        }
    }
