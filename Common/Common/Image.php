...
class Common_Image {
    ...
    function showImg($imgFile, $text='', $x='10', $y='10', $alpha='50') {
        ...
        $info = Common_Image::getImageInfo($imgFile);
        if ($info !== false) {
            ...
            if ($im) {
                ...
                if (!empty($text)) {
                    ...
                    if (is_file($text) && file_exists($text)) {//判断$text是否是图片路径
                        // 取得水印信息
                        $textInfo = Common_Image::getImageInfo($text);
    ...
    static function thumb($image, $thumbname, $type='', $maxWidth=200, $maxHeight=50, $interlace=true) {
        // 获取原图信息
        $info = Common_Image::getImageInfo($image);
        if ($info !== false) {
            $srcWidth = $info['width'];
    ...
    static function buildString($string, $rgb=array(), $filename='', $type='png', $disturb=1, $border=true) {
        ...
        Common_Image::output($im, $type, $filename);
    }
    ...
    static function buildImageVerify($length=4, $mode=1, $type='png', $width=48, $height=22, $verifyName='verify') {
        $randval = Common_String::randString($length, $mode);
...
//        imagerectangle($im, 0, 0, $width - 1, $height - 1, $borderColor);
        ...
        for ($i = 0; $i < $length; $i++) {
            ...
            $codex = Common_String::msubstr($code, $i, 1);
            imagettftext($im, mt_rand(15, 19), mt_rand(-60, 60), intval($width/$length-5) * $i + 20, mt_rand(25, 30), $fontcolor, $fontface, $codex);
        }
        Common_Image::output($im, $type);
    }
    ...
    static function output($im, $type='png', $filename='') {
        ob_end_clean(); //防止出现'图像因其本身有错无法显示'的问题
